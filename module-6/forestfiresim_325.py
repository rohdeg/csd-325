    ...
    while True:  # Main program loop.
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                # Protect water from modification
                if forest[(x, y)] == WATER:
                ...
                if ((forest[(x, y)] == EMPTY)
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            ...
                            and forest.get((x + ix, y + iy)) != WATER:  # Added this to ensure fire cannot spread to water
...
def createNewForest():
    ...
    #Create rectangle coords for lake
    lake_start_x = WIDTH // 2 - 10
    lake_end_x = WIDTH // 2 + 10
    lake_start_y = HEIGHT // 2 - 3
    lake_end_y = HEIGHT // 2 + 3

    for x in range(WIDTH):
        for y in range(HEIGHT):
            # Modify IF structure to also plot water
